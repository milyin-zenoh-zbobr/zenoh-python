    ...
    fn reply(
        ...
        if congestion_control.is_some() {
            import!(py, warnings.warn).call1((
                "The 'congestion_control' parameter is deprecated and will be removed in a future release",
            ))?;
        }
        if priority.is_some() {
            import!(py, warnings.warn).call1((
                "The 'priority' parameter is deprecated and will be removed in a future release",
            ))?;
        }
        let build = build!(
